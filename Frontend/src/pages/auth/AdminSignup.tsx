...
interface FormData {
  ...
  phone: string;
  adminLevel: 'ADMIN' | 'SUPPORT_ADMIN';
}
...
const AdminSignup = () => {
  ...
  try {
    ...
    if (!response.ok) {
      // If response is not ok, assume it contains an error message
      const errorMessage = responseBody?.message || 'Signup failed';
      throw new Error(errorMessage);
    }

    const data: { admin: Admin; requiresApproval: boolean } = responseBody;

    toast({
      title: 'Account Created Successfully!',
      description: data.requiresApproval
        ? `Your ${formData.adminLevel.toLowerCase().replace('_', ' ')} account is pending approval.`
        : `Your ${formData.adminLevel.toLowerCase().replace('_', ' ')} account has been created.`,
    });

    setTimeout(() => {
      if (data.requiresApproval) {
        navigate('/admin/pending-approval', {
          state: { email: data.admin.email, name: data.admin.name, adminLevel: data.admin.adminLevel },
        });
      } else {
        ...
        <div className="w-full max-w-lg mx-auto">
          ...
          <div className="bg-white p-8 rounded-lg shadow-sm border border-gray-100">
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700 mt-1">
                    Your account will need approval from a Root Administrator.
                  </p>
                </div>
              </div>
            </div>
            <form onSubmit={handleSubmit} autoComplete="on">
              ...
              <div className="mb-4">
                <Label htmlFor="adminLevel" className="block font-medium mb-2">Administrator Type *</Label>
                <select
                  id="adminLevel"
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {formData.adminLevel === 'ADMIN' ? 'Full administrative access' : 'Customer support functions'}
                </p>
              </div>
              <div className="mb-4">
